...
interface OpenStreetMapWithDensityProps {
  apartments: any[];
...
// Custom marker icon with theme colors and apartment count
const createCustomDensityMarker = (apartmentCount: number) => {
  if (typeof window === 'undefined') return null;
  
  const L = require('leaflet');
  
  // Determine marker size and opacity based on apartment count
  const size = Math.min(Math.max(apartmentCount / 2000, 12), 50);
  const opacity = Math.min(apartmentCount / 50000, 0.9);
  
  // Format apartment count for display
  const displayCount = apartmentCount > 10000 
    ? Math.floor(apartmentCount / 1000) + 'K'
    : apartmentCount > 1000 
    ? Math.floor(apartmentCount / 100) / 10 + 'K'
...
const OpenStreetMapWithDensity: React.FC<OpenStreetMapWithDensityProps> = ({
  apartments,
  onMarkerClick
}) => {
    ...
    <div className="relative w-full h-full">
        ...
        >
          ...
          {densityData.map((marker) => (
            <Marker
              ...
              icon={createCustomDensityMarker(marker.apartmentCount)}
              eventHandlers={{
                click: () => {
                  if (onMarkerClick) {
            ...
            >
              <Popup>
                <div style={{ padding: '10px', minWidth: '200px' }}>
                  <h3 style={{ margin: '0 0 8px 0', color: '#475569', fontSize: '16px', fontWeight: 'bold' }}>
                    {marker.city}, {marker.state}
                  </h3>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                    <span style={{ color: '#64748B' }}>Available Apartments:</span>
                    <span style={{ fontWeight: 'bold', color: '#1E293B' }}>
                      {marker.apartmentCount.toLocaleString()}
                    </span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <span style={{ color: '#64748B' }}>Average Price:</span>
                    <span style={{ fontWeight: 'bold', color: '#059669' }}>
                      ${marker.averagePrice.toLocaleString()}/mo
                    </span>
                  </div>
                  <button 
                    onClick={() => {
                      window.location.href = `/apartments?city=${encodeURIComponent(marker.city)}&state=${marker.state}`;
                    }}
                    style={{
                      width: '100%',
      ...
      <div className="absolute bottom-4 left-4 bg-white shadow-lg rounded-lg px-4 py-2 z-10">
        ...
        <p className="text-xs text-gray-500">
          Powered by OpenStreetMap • No billing required
        </p>
      </div>
    </div>
  );
...
export default OpenStreetMapWithDensity;

...
export async function POST(request: NextRequest) {
  try {
    const { email, firstName, lastName, paymentIntentId, amount, submittedAt, requestId } = await request.json();
    ...
    const emailContent = `
      ...
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color-scheme: light dark; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div class="content-area" style="background: white; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
          ...
          <p class="text-primary" style="margin: 0 0 30px 0; font-size: 16px;">We've received your apartment search request.</p>
          
          <h3 class="text-primary" style="margin-bottom: 15px;">Next Steps</h3>
          <div style="margin-bottom: 15px;">
            <strong class="text-primary">1. Request Review (24-48 hours)</strong><br>
            <span class="text-secondary">Our team will review your preferences and start the search.</span>
          </div>
          <div style="margin-bottom: 15px;">
            <strong class="text-primary">2. Apartment Matching</strong><br>
            <span class="text-secondary">We'll match you with suitable apartments based on your criteria.</span>
          </div>
          <div style="margin-bottom: 15px;">
            <strong class="text-primary">3. Property Recommendations</strong><br>
            <span class="text-secondary">You'll receive personalized apartment recommendations via email.</span>
          </div>
        </div>
    ...
    console.log(`📧 Apartment finder confirmation email would be sent to: ${email}`);
    console.log(`🏠 Request submitted by: ${firstName} ${lastName}`);
    console.log(`💳 Payment ID: ${paymentIntentId}`);
    console.log(`📋 Request ID: ${requestId}`);

    return NextResponse.json({
      success: true,
      message: 'Apartment finder confirmation email sent successfully'
    });

  } catch (error) {
    console.error('Error sending apartment finder confirmation email:', error);
    return NextResponse.json(
